                ...
                for (int k = 0; k < Main.npc.Length - 1; k++)
                {
                    ...
                    if (Main.npc[k].active && (Main.npc[k].boss || Main.npc[k].type == NPCID.EaterofWorldsHead || Main.npc[k].type == NPCID.EaterofWorldsBody || Main.npc[k].type == NPCID.EaterofWorldsTail))
                    {
                        ...
                        if (careAboutRange)
                        {
                            ...
                            if (Math.Abs(Player.Center.X - Main.npc[k].Center.X) + Math.Abs(Player.Center.Y - Main.npc[k].Center.Y) < 4000f) return true;
                            else return false;
                        } 
                        else return true;
                    }
        ...
        public override void UpdateDead()
        {
            ...
            if (!bossAlive)
            {
                ...
                if (config.UseNumbers)
                {
                    ...
                    if (Main.expertMode && Player.respawnTimer > config.ExpertRespawnTime * 60) Player.respawnTimer = config.ExpertRespawnTime * 60;

                    else if (Player.respawnTimer > config.NormalRespawnTime * 60) Player.respawnTimer = config.NormalRespawnTime * 60;
                } else
                {
